...
def main():
    ...
    if not st.session_state.messages:
        ...
        example_questions = [
            "מהי מכסת ימי החופשה השנתית לעובד?",
            "מה שיעור שעות העבודה השבועיות?",
            "מה שיעור פיצויי פיטורים לשנת עבודה?",
            "כמה זמן מראש יש להודיע על התפטרות?",
            "מהם ימי הבחירה שמגיעים לעובד?",
            "מה גובה קצובת ההבראה ליום?",
        ]
        cols = st.columns(3)
